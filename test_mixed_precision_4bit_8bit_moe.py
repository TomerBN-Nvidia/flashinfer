...
CONFIGURATIONS TESTED:
    1. NVFP4 weights × FP8 activations
       - Expected: FAIL on SM90 (NVFP4 requires SM100+)
       
    2. MXFP4 weights × FP8 activations  
       - Expected: FAIL on SM90 (MXFP4×FP8 requires SM100+)
       
    3. Int4 weights × FP8 activations (W4A8)
...
def print_config(config: MoEConfig):
    ...
    print(f"  Activation Format:  FP8 (E4M3)")
...
# ==============================================================================
# Test Functions for Each Weight Format
# ==============================================================================

def test_nvfp4_x_fp8(config: MoEConfig) -> Dict[str, Any]:
    """
    ...
    """
    print_separator("TEST: NVFP4 × FP8 MoE")
    print_config(config)
    print()
    ...
    try:
        ...
        # Create FP8 activations
        x_bf16 = torch.randn(m, k, dtype=torch.bfloat16, device=device)
        x_fp8, x_scale = dynamic_per_tensor_fp8_quant(x_bf16)
        
        print(f"\n  Created FP8 activations: shape={x_fp8.shape}, dtype={x_fp8.dtype}")
        ...
        print(f"\n  Calling cutlass_fused_moe with NVFP4 weights × FP8 activations...")
        print(f"  ⚠️  This is expected to FAIL on SM90 (Hopper)")
        print()
        
        # The key call - attempting NVFP4 × FP8 MoE
        # Note: We're using FP8 input directly without the typical NVFP4 activation path
        # to force the error condition
        _ = cutlass_fused_moe(
            x_fp8,  # FP8 activations (8-bit)
        ...
        result["success"] = True
        print("  ✓ UNEXPECTED SUCCESS: NVFP4 × FP8 completed without error!")
...
def test_mxfp4_x_fp8(config: MoEConfig) -> Dict[str, Any]:
    """
    Test MXFP4 (4-bit) weights × FP8 (8-bit) activations.
    ...
    """
    print_separator("TEST: MXFP4 × FP8 MoE")
    print_config(config)
    print()
    ...
    try:
        ...
        # Create input and quantize to FP8
        x = torch.randn(m, k, dtype=torch.bfloat16, device=device)
        
        print(f"  Quantizing activations to MXFP8...")
        # MXFP8 quantization
        mxfp8_x, mxfp8_x_sf = mxfp8_quantize(x, True, 32)
        print(f"  MXFP8 activations: shape={mxfp8_x.shape}, dtype={mxfp8_x.dtype}")
        ...
        # Prepare quant_scales for MXFP4 × MXFP8
        ...
        print(f"\n  Calling cutlass_fused_moe with MXFP4 weights × MXFP8 activations...")
        print(f"  ⚠️  This is expected to FAIL on SM90 (Hopper)")
        print()
        
        # The key call - attempting MXFP4 × FP8 MoE
        _ = cutlass_fused_moe(
            mxfp8_x,  # MXFP8 activations (8-bit)
            selected_experts.to(torch.int),
            routing_weights,
            ...
            config.output_dtype,
            quant_scales=quant_scales,
            input_sf=mxfp8_x_sf,  # MXFP8 scale factors
            use_mxfp8_act_scaling=True,  # Indicate MXFP8 activations
            output=output,
            activation_type=ActivationType.Relu2,  # Non-gated
        )
        
        result["success"] = True
        print("  ✓ UNEXPECTED SUCCESS: MXFP4 × FP8 completed without error!")
...
def test_int4_x_fp8(config: MoEConfig) -> Dict[str, Any]:
    """
    Test Int4 (4-bit) weights × FP8 (8-bit) activations (W4A8 path).
    ...
    """
    print_separator("TEST: Int4 × FP8 MoE (W4A8)")
    print_config(config)
    print()
    
    result = {
        "config": "Int4 × FP8 (W4A8)",
        "success": False,
        "error_type": None,
        "error_message": None,
        "error_location": None,
    ...
    try:
        ...
        print(f"\n  Calling cutlass_fused_moe with Int4 weights × implicit FP8 activations...")
        print(f"  Using Swiglu (gated) activation for W4A8 path")
        print(f"  Note: This may succeed on SM90 via the W4A8 path")
        print()
        
        # The key call - attempting Int4 × FP8 (W4A8) MoE
        # NOTE: W4A8 uses Swiglu (default), NOT Relu2!
        _ = cutlass_fused_moe(
            x,  # BF16/FP16 input (internally quantized to FP8)
        ...
        result["success"] = True
        print("  ✓ SUCCESS: Int4 × FP8 (W4A8) completed!")
...
def test_int4_x_fp8_relu2(config: MoEConfig) -> Dict[str, Any]:
    """
    Test Int4 (4-bit) weights × FP8 (8-bit) activations with Relu2 (non-gated).
    
    EXPECTED BEHAVIOR:
        This tests whether W4A8 supports non-gated (Relu2) activation.
        For non-gated MoE:
    ...
    """
    print_separator("TEST: Int4 × FP8 MoE with Relu2 (Non-Gated)")
    print_config(config)
    print()
    
    result = {
        "config": "Int4 × FP8 (W4A8) + Relu2",
        "success": False,
        "error_type": None,
        "error_message": None,
        "error_location": None,
    ...
    try:
        ...
        print(f"\n  Calling cutlass_fused_moe with Int4 weights × FP8 + Relu2 (non-gated)...")
        print(f"  This tests if W4A8 supports non-gated activation")
        print()
        
        # The key call - attempting Int4 × FP8 with Relu2
        _ = cutlass_fused_moe(
            x,
            selected_experts.to(torch.int32),
            routing_weights,
        ...
        result["success"] = True
        print("  ✓ SUCCESS: Int4 × FP8 + Relu2 completed!")
        print(f"    Output shape: {output.shape}")
        print(f"    Output dtype: {output.dtype}")
        print(f"    Output sample: {output[0, :5]}")
...
def main():
    """Run all mixed-precision 4-bit × 8-bit MoE tests."""
    ...
    # Run all tests
    # ==========================================================================
    # 4-bit weights × 8-bit (FP8) activations
    # ==========================================================================
    ...
    # Test 3: Int4 × FP8 with Swiglu (gated) - standard W4A8 path
    config.weight_format = WeightFormat.INT4
    results.append(test_int4_x_fp8(config))
    
    # Test 4: Int4 × FP8 with Relu2 (non-gated) - tests non-gated W4A8 support
    results.append(test_int4_x_fp8_relu2(config))
    ...
    if sm_version >= 90 and sm_version < 100:
        unexpected_successes = [r for r in results if r["success"] and r["config"] in ["NVFP4 × FP8", "MXFP4 × FP8"]]
